...
根据课上关于栈结构的note，以及return address和上一级frame pointer与当前frame pointers的固定偏移量（-8和-16），即可一层层向上跳，直到到达PGROUNDDOWN为止。
...
实现两个函数`sigalarm`和`sigreturn`，即调用`sigalarm(interval, handler)`后，每`interval` 个CPU时间，就会调用handler函数，然后通过`sigreturn` 返回。通过`usertrap`中的时间中断实现（每一个CPU时间，都会触发时间中断一次），要避免出现在执行`handler`次再次调用`handler`的情况，并且能够在中断结束后恢复此前的运行状态，继续运行。

具体实现方式是在`proc`中增加相应的变量，包括`interval`对应的`alarm_ticks`，`handler`对应的`alarm_handler`，以及当前时间计数器`cur_alarm_ticks`，和当前是否正在运行`handler`的`alarm_enabled`，最后一个是为了恢复所有寄存器状态而对`trapframe`的备份`alarm_trapframe`。
